    ...
    if (gameState.phase === 'reveal' && isMyTurn) {
      if (!pendingAction) {
        ...
        if (pendingAction.type === 'select_3x3') {
          ...
          for (let i = 0; i < 2; i++) {
            for (let j = 0; j < 2; j++) {
              if (!gameState.board[r+i][c+j].revealed) hasUnrevealed = true;
            }
          }
          ...
          for (let j = 0; j < gameState.boardCols; j++) {
            if (!gameState.board[r][j].revealed) hasUnrevealed = true;
          }
          if (!hasUnrevealed) return;
          client.sendIntent({ type: 'action_roulette_select', cells: [{r, c}] });
          ...
          for (let i = 0; i < gameState.boardRows; i++) {
            if (!gameState.board[i][c].revealed) hasUnrevealed = true;
          }
          if (!hasUnrevealed) return;
          client.sendIntent({ type: 'action_roulette_select', cells: [{r, c}] });
        } else if (pendingAction.type === 'select_1_private') {
          if (gameState.board[r][c].revealed) return; // Private hint should probably be on unrevealed cell too? Wait, the user didn't specify this, but it makes sense. Let's just leave it as is or add it. Actually user said "翻2*2，翻一行，翻一列至少需要翻开一个格子！". So I'll just do those 3.
          client.sendIntent({ type: 'action_roulette_select', cells: [{r, c}] });
  ...
  const getCellContent = (cell: CellData) => {
    ...
    if (cell.revealed) {
      if (cell.bombs.length > 0) {
        const getBombOffset = (index: number, total: number) => {
          if (total === 1) return { x: 0, y: 0 };
          if (total === 2) return index === 0 ? { x: -4, y: -4 } : { x: 4, y: 4 };
          if (total === 3) {
            if (index === 0) return { x: 0, y: -5 };
            if (index === 1) return { x: -5, y: 4 };
            return { x: 5, y: 4 };
          }
          if (index === 0) return { x: -4, y: -4 };
          if (index === 1) return { x: 4, y: -4 };
          if (index === 2) return { x: -4, y: 4 };
          return { x: 4, y: 4 };
        };
        ...
        return (
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            {cell.bombs.map((playerId, i) => {
              ...
              const offset = getBombOffset(i, cell.bombs.length);
              return (
                <motion.div
                  key={i}
                  initial={{ scale: 0, rotate: -30 }}
                  animate={{ scale: 1, x: offset.x, y: offset.y, rotate: (i - (cell.bombs.length-1)/2) * 15 }}
